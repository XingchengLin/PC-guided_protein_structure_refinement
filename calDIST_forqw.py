        ...
        if (is_number(line1[3])):
            ...
            for j in my_lt_range(i + 3, length, 1):
                ...
                try:
                    ...
                    # Only write down the pairs existed in the crystal structure;
                    # Note: We only do this for the calculation of QW, not in the bias!!!
                    ResidExisted = list(range(1, 100000, 1))
                    if ((int(line1[2]) in ResidExisted) and (int(line2[2]) in ResidExisted)):
