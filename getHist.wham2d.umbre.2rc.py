...
for i in my_le_range(lowT, highT, stepT):
    for j in umbrelist1:
        for k in reflist1:
            for l in umbrelist2:
                for m in reflist2:
                    histfile = histName + '/T' + str(i) + '/umbre1.fc' + str(j) + '/r10_' + str(k) + '/umbre2.fc' + str(l) + '/r20_' + str(m) + '/hist.dat'
                    numFiles += 1
                    with open(histfile, 'r') as infile:
                        for line in iter(infile.readline, ''):
                            ...
                            if float(tmp[3]) > mostUm1:
                                mostUm1 = float(tmp[3])
                            if float(tmp[4]) < leastUm2:
                                leastUm2 = float(tmp[4])
                            if float(tmp[4]) > mostUm2:
                                mostUm2 = float(tmp[4])

print(str(leastEne) + "\t" + str(mostEne) + "\t" + str(leastQ1) + "\t" + str(mostQ1) + "\t" + str(leastQ2) + "\t" + str(mostQ2) + "\t" + str(leastUm1) + "\t" + str(mostUm1) + "\t" + str(leastUm2) + "\t" + str(mostUm2))
stepE = 100.0
stepQ1 = 0.01
stepQ2 = 0.01
stepUm1 = 0.01
...
outfile.write("\n")
leastUm2 = round(leastUm2, precision)
numUmBins = int((mostUm2 - leastUm2) / stepUm2 + 0.5)
outfile.write("numBins" + "\t" + str(numUmBins) + "\n")
outfile.write("start" + "\t" + str(leastUm2) + "\n")
outfile.write("step" + "\t" + str(stepUm2) + "\n")
    ...
    for j in umbrelist1:
        for k in reflist1:
            for l in umbrelist2:
                for m in reflist2:
                    histfile = histName + '/T' + str(i) + '/umbre1.fc' + str(j) + '/r10_' + str(k) + '/umbre2.fc' + str(l) + '/r20_' + str(m) + '/hist.dat'
                    outfile.write("name" + " " + histfile + " " + "temp" + " " + str(i) + " " + "umbrella_k" + " " + str(j) + " " + "umbrella_0" + " " + str(k)  + " " + "umbrella_k" + " " + str(l) + " " + "umbrella_0" + " " + str(m) + " " + "\n")
